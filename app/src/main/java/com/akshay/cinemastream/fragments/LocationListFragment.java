...
import com.akshay.cinemastream.adapters.LocationRecyclerViewAdapter;
...
public class LocationListFragment extends Fragment {
    ...
    public static LocationListFragment newInstance(String movieId) {
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        Call<List<LocationDetail>> call = CinemaApiClient.getApiService().loadMovieFilmingLoctions(movieId, AppConstants.TOKEN);
        call.enqueue(new Callback<List<LocationDetail>>() {
            @Override
            public void onResponse(Response<List<LocationDetail>> response, Retrofit retrofit) {
                if (response.isSuccess()) {
                    if (response.body() != null) {

                        if (response.body() != null && !response.body().isEmpty()) {
                            errorLayout.setVisibility(View.GONE);
                            mAdapter = new RecyclerViewMaterialAdapter(new LocationRecyclerViewAdapter(response.body()));
